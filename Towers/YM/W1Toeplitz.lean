...
private lemma r₇_pos : 0 < r₇ := by unfold r₇; positivity
...
private lemma entry₇_nonneg (k : ℤ) (i j : Fin 3) :
  ...
  exact div_nonneg (pow_nonneg (div_nonneg (by positivity) two_pos.le) _) (by positivity)

private lemma entry₇_le_pow_mul (k : ℤ) (i j : Fin 3) :
    (toeplitzReal βR₇ k) i j ≤ r₇ ^ ((i : ℤ) - j - k).natAbs * C_exp₇ := by
  simp only [toeplitzReal, Matrix.of_apply]
  apply le_trans (besselI_series_le_exp_bound _ _ (by positivity))
...
private lemma index₇_lower (k : ℤ) (i j : Fin 3) :
  ...
  calc k.natAbs
    ...
    _ ≤ (i - j - k).natAbs + 2 := Nat.add_le_add_left hij _
  omega
...
private lemma g₇_nonneg (k : ℤ) : 0 ≤ g₇ k := by unfold g₇; positivity
...
private lemma tsum₇_det_split :
  ...
  (Finset.sum_add_tsum_compl S6k7 summable_toeplitz_det₇).symm
...
theorem w1_seven_gt_eighth : 1 / 8 < w1_weyl_series βR₇ := by
  ...
  have hFT_nn : (0 : ℝ) ≤ ↑finite_7_lo_sum - ↑tail_7_ub := by
    have : (0 : ℚ) < finite_7_lo_sum - tail_7_ub := by decide
